...
public class VulkanDevice implements GpuDeviceBackend {
    ...
    public VulkanDevice(
        ...
        // 在 VulkanDevice 构造函数中，初始化 pipelineCache
        this.pipelineCachePath = Paths.get(System.getProperty("user.dir"), CACHE_FILE_NAME);
        this.pipelineCache = this.createPipelineCache();
    }
            ...
            if (Files.exists(this.pipelineCachePath)) {
                try {
                    ...
                    if (bytes.length > 0) {
                        ...
                        LOGGER.info("Loaded Vulkan pipeline cache from disk ({} bytes)", bytes.length);
                    }
                } catch (IOException e) {
                    LOGGER.warn("Failed to load Vulkan pipeline cache from disk", e);
                }
            }

            LongBuffer pointer = stack.callocLong(1);
            int result = VK12.vkCreatePipelineCache(this.vkDevice, createInfo, null, pointer);
            VulkanUtils.crashIfFailure(this, result, "Failed to create pipeline cache");
    ...
    private void savePipelineCache() {
        if (this.pipelineCache == 0L) return;
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            long cacheSize = sizePtr.get(0);
            if (cacheSize == 0L) {
                LOGGER.debug("Pipeline cache is empty, skipping save");
                return;
            }

            if (cacheSize > Integer.MAX_VALUE) {
                LOGGER.warn("Pipeline cache too large ({} bytes), skipping save", cacheSize);
                return;
            }

            // 分配缓冲区并获取数据
            ByteBuffer cacheData = MemoryUtil.memAlloc((int) cacheSize);
            try {
                result = VK12.vkGetPipelineCacheData(this.vkDevice, this.pipelineCache, sizePtr, cacheData);
                if (result != 0) {
                ...
                // 写入磁盘
                Files.write(this.pipelineCachePath, cacheData.array());
                LOGGER.info("Saved Vulkan pipeline cache to disk ({} bytes)", cacheData.remaining());
            } catch (IOException e) {
                LOGGER.warn("Failed to save pipeline cache to disk", e);
            } finally {
                MemoryUtil.memFree(cacheData);
            }
        }
    ...
    public void close() {
        ...
        if (this.pipelineCache != 0L) {
            VK12.vkDestroyPipelineCache(this.vkDevice, this.pipelineCache, null);
        }
    ...
    public VulkanGpuBuffer createBuffer(final @Nullable
            Supplier<String> label, final @GpuBuffer.Usage int usage, final long size) {
        return new VulkanGpuBuffer.Direct(this, label, usage, size, this.isIntegratedIntelMoltenVK);
    }

    @Override
    public GpuBuffer createBuffer(final @Nullable
            Supplier<String> label, final @GpuBuffer.Usage int usage, final ByteBuffer data) {
