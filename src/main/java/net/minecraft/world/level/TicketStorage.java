...
public class TicketStorage extends SavedData {
    ...
    private Set<ChunkPos> chunksWithForcedTickets = new ObjectOpenHashSet<>();
    ...
    private static TicketStorage fromPacked(final List<Pair<ChunkPos, Ticket>> tickets) {
        Object2ObjectOpenHashMap<ChunkPos, List<Ticket>> ticketsToLoad = new Object2ObjectOpenHashMap<>(4);
        ...
        return new TicketStorage(ticketsToLoad, new Object2ObjectOpenHashMap<>(4));
    }
    ...
    private static void forEachTicket(final BiConsumer<ChunkPos, Ticket> output, final Object2ObjectOpenHashMap<ChunkPos, List<Ticket>> tickets) {
        for (Object2ObjectOpenHashMap.Entry<ChunkPos, List<Ticket>> entry : Object2ObjectMaps.fastIterable(tickets)) {
    ...
    public void activateAllDeactivatedTickets() {
        for (Object2ObjectOpenHashMap.Entry<ChunkPos, List<Ticket>> entry : Object2ObjectMaps.fastIterable(this.deactivatedTickets)) {
            for (Ticket ticket : entry.getValue()) {
    ...
    public void removeTicketIf(final TicketStorage.TicketPredicate predicate, final @Nullable Object2ObjectOpenHashMap<ChunkPos, List<Ticket>> removedTickets) {
        ObjectIterator<Object2ObjectOpenHashMap.Entry<ChunkPos, List<Ticket>>> ticketsPerChunkIterator =
            this.tickets.object2ObjectEntrySet().fastIterator();
        boolean removedForced = false;

        while (ticketsPerChunkIterator.hasNext()) {
            Object2ObjectOpenHashMap.Entry<ChunkPos, List<Ticket>> entry = ticketsPerChunkIterator.next();
    ...
    public void replaceTicketLevelOfType(final int newLevel, final TicketType ticketType) {
        ...
        for (Object2ObjectOpenHashMap.Entry<ChunkPos, List<Ticket>> entry : this.tickets.object2ObjectEntrySet()) {
            for (Ticket ticket : entry.getValue()) {
    ...
    private Set<ChunkPos> getAllChunksWithTicketThat(final Predicate<Ticket> ticketCheck) {
        ...
        for (Object2ObjectOpenHashMap.Entry<ChunkPos, List<Ticket>> entry : Object2ObjectMaps.fastIterable(this.tickets)) {
            for (Ticket ticket : entry.getValue()) {
