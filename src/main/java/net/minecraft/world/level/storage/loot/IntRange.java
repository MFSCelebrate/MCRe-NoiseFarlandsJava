...
public class IntRange implements LootContextUser {
   private static final Codec<IntRange> RECORD_CODEC = RecordCodecBuilder.create(
      i -> i.group(
            NumberProviders.DIRECT_CODEC.optionalFieldOf("min").forGetter(r -> Optional.ofNullable(r.min)),
            NumberProviders.DIRECT_CODEC.optionalFieldOf("max").forGetter(r -> Optional.ofNullable(r.max))
         )
