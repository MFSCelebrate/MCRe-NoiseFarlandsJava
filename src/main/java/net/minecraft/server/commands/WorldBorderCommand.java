...
public class WorldBorderCommand {
    ...
    private static final SimpleCommandExceptionType ERROR_TOO_BIG = new SimpleCommandExceptionType(
        Component.translatable("commands.worldborder.set.failed.big", Long.MAX_VALUE)
    );
    private static final SimpleCommandExceptionType ERROR_TOO_FAR_OUT = new SimpleCommandExceptionType(
        Component.translatable("commands.worldborder.set.failed.far", Long.MAX_VALUE)
    );
            ...
            Commands.literal("worldborder")
                ...
                .then(
                    Commands.literal("add")
                        .then(
                            Commands.argument("distance", DoubleArgumentType.doubleArg(-Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY))
                                .executes(
                                    c -> setSize(
    ...
    private static int setCenter(final CommandSourceStack source, final Vec2 center) throws CommandSyntaxException {
        ...
        } else if (!(Math.abs(center.x) > Double.POSITIVE_INFINITY) && !(Math.abs(center.y) > Double.POSITIVE_INFINITY)) {
